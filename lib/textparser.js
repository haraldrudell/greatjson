...
function getParser(text) {
	...
	function skipThisCharacter(ch, expected) {
		var result
		if (peekAtNextCharacter() != ch) result = reportError(unparsed.length ? badToken : suddenEnd, expected)
		else skipCharacter()
		return result
	}
	...
	function getFullString() {
		...
		if (peekAtNextCharacter() != '"') {
			result = reportError(unparsed.length ? badToken : suddenEnd, 'double quote character')
		} else {
			skipCharacter()
			result = getString()
		}
	...
	function getString() {
		...
		if (match) {
			...
			result = match[0].replace(regexps.jsonStringEscapes, function(matched, escapeChar, escapeHex) {
	...
	function getSimpleToken() {
		...
		if (match) {
			var skipc = 0
			if (match[0]) skipc += match[0].length
			if (match[1]) {
				skipc += match[1].length
				result = match[1]
			}
			unparsed = unparsed.substring(skipc)
		}
		if (!result) {
console.log(arguments.callee.name, unparsed.length)
			result = reportError(unparsed.length ? badToken : suddenEnd, text, unparsed)
		}
		return result
	}
