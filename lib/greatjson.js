	...
	*/
	...
	function getObject() {
		...
		if (parser.peekAtNextCharacter() != '}') for (;;) {
			...
			if ((nextValue = parser.skipThisCharacter(':', 'json object colon separator')) && (result = nextValue)) break
			...
			if (nextValue = parser.skipThisCharacter(',', 'object comma') && (result = nextValue)) break
	...
	function getArray() {
		...
		if (parser.peekAtNextCharacter() != ']') for (;;) {
			...
			if (nextValue = parser.skipThisCharacter(',', 'array comma') && (result = nextValue)) break
		} else parser.skipCharacter()
		return result
	}
